...
async function getProduct() {
  try {
    const response = await fetch(
      "https://localhost:44361/api/Products/GetAllProducts"
    );
...
function displayPaginatedCategories(Products, page, itemsPerPage) {
  ...
  paginatedItems.forEach((product) => {
    productContainer.innerHTML += `
            <tr>
            <td><img src="https://localhost:44361/${product.productImage1}" alt="${product.productName}" class="img-fluid" style="height: 50px;"></td>
...
async function deleteProduct(id) {
  if (confirm("Are you sure you want to delete this Product?")) {
    try {
      const response = await fetch(
        `https://localhost:44361/api/Products/DeleteProduct/${id}`,
        {
          method: "DELETE",
          headers: {
