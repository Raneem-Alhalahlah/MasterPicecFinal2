...
function displayPaginatedCategories(categories, page, itemsPerPage) {
  ...
  paginatedItems.forEach((category) => {
    categoryContainer.innerHTML += `
            <tr>
                <td><img src="https://localhost:44361/${category.categoryImage}" alt="${category.categoryName}" class="img-fluid" style="height: 50px;"></td>
...
async function deleteCategory(categoryId) {
  if (confirm("Are you sure you want to delete this category?")) {
    try {
      const response = await fetch(
        `https://localhost:44361/api/Categories/DeleteCategory/${categoryId}`,
        {
          method: "DELETE",
          headers: {
